...
class WorkflowHttpE2ECompanySpec extends BaseLiferaySpec {
	...
	def 'execute creates a company, user, and organization via HTTP JSON'() {
		...
		// CompanyService is blacklisted from JSONWS; use the headless portal-instances
		// path-based getter. The API exposes the webId as `portalInstanceId`.
		Map<String, Object> createdCompany = headlessGet(
