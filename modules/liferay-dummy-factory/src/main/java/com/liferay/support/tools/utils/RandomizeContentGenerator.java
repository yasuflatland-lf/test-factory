 ...
 */
@Component(service = RandomizeContentGenerator.class)
public class RandomizeContentGenerator {
	...
	public List<String> generateLinks(String linkLists) {
		...
		List<String> links = new ArrayList<>(Arrays.asList(rows));
	 ...
	 */
	public List<Integer> generateRandomIndex(
		int totalParagraphs, int randomAmount) {

		if (totalParagraphs <= 0 || randomAmount <= 0) {
			return new ArrayList<Integer>();
		}

		List<Integer> integers = IntStream.range(
			0, totalParagraphs
		).boxed(
		).collect(
			Collectors.toList()
	 ...
	 */
	public String insertRandomLinksInContents(
		List<String> paragraphes, List<String> links, List<Integer> rndIndex) {
	 ...
	 */
	public List<String> generateRandomParagraphes(
		String locale, int totalParagraphs) {
	...
	public String generateRandomContents(
		String locale, int totalParagraphs, int randomAmount,
		String linkLists) {

		List<String> paragraphes = generateRandomParagraphes(
			locale, totalParagraphs);

		List<Integer> rndIndex = generateRandomIndex(
			totalParagraphs, randomAmount);
