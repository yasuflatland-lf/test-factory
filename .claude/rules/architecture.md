...
**DataListProvider SPI** -- Dropdown data sources (organizations, roles, user-groups, sites, site-templates, vocabularies, mb-categories, mb-threads, etc.) are served by `DataListProvider` implementations discovered dynamically via OSGi `@Reference(cardinality=MULTIPLE, policy=DYNAMIC)`. New data types are added by creating a `@Component(service=DataListProvider.class)` class in `service/datalist/` — no changes to `DataListResourceCommand` needed.
