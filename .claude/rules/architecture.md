...
**MVCResourceCommands** -- Entity creation is handled by per-entity resource commands: `/ldf/org` (Organization), `/ldf/user` (User), `/ldf/role` (Role), `/ldf/site` (Site), `/ldf/page` (Layout), `/ldf/wcm` (Web Content), `/ldf/doc` (Document) with `/ldf/doc/upload` for file upload, `/ldf/vocabulary` (Vocabulary), and `/ldf/category` (Category). `/ldf/data` (`DataListResourceCommand`) serves dropdown data for organizations, roles, user-groups, site-roles, org-roles, sites, site-templates, and vocabularies. `/ldf/progress` (`ProgressResourceCommand`) reports batch-creation progress.
...
**DataListProvider SPI** -- Dropdown data sources (organizations, roles, user-groups, sites, site-templates, vocabularies, etc.) are served by `DataListProvider` implementations discovered dynamically via OSGi `@Reference(cardinality=MULTIPLE, policy=DYNAMIC)`. New data types are added by creating a `@Component(service=DataListProvider.class)` class in `service/datalist/` — no changes to `DataListResourceCommand` needed.
...
| **No container reuse (`withReuse(false)`)** | Although Liferay startup is slow (~8 min), reusing containers lets state from a prior run (users, roles, password changes) leak into the next run and hide regressions. Each test run starts fresh. |
| **JSONWS-first verification** | Post-condition checks go through Liferay JSONWS (`/api/jsonws/...`) rather than Playwright/UI navigation. JSONWS is faster, deterministic, and independent of Control Panel rendering. Playwright is reserved for genuinely UI-specific behavior. |
